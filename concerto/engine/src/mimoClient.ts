...
export class MimoClient {
  ...
  async generateCode(prompt: string): Promise<string> {
    const monoPath = process.env.GIT_REPO_PATH || process.cwd();
    let messages: any[] = [
      { 
        role: "system", 
        content: `You are MiMo, an expert AI engineering assistant working on a PHP/Symfony repository.
...
When finished, output a summary of all files you created or modified.` 
  ...
  private async handleToolCalls(content: string): Promise<string> {
    let results = "";
