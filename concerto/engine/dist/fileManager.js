"use strict";
/**
 * File Manager - Handles read/write operations for .md files
 * Supports: Specs, Prompts, Maestros, Workflows
 */
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.workflowsManager = exports.maestrosManager = exports.promptsManager = exports.specsManager = void 0;
exports.parseFrontmatter = parseFrontmatter;
exports.createFrontmatter = createFrontmatter;
exports.writeMarkdownFile = writeMarkdownFile;
exports.readMarkdownFile = readMarkdownFile;
exports.listMarkdownFiles = listMarkdownFiles;
exports.ensureDir = ensureDir;
exports.deleteFile = deleteFile;
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const yaml_1 = __importDefault(require("yaml"));
const CONFIG_ROOT = path_1.default.join(process.cwd(), '..', 'concerto-config');
const MAESTROS_ROOT = path_1.default.join(process.cwd(), '..', 'maestros');
const WORKFLOWS_ROOT = path_1.default.join(process.cwd(), '..', 'workflows');
/**
 * Parse YAML frontmatter from markdown
 */
function parseFrontmatter(content) {
    const lines = content.split('\n');
    if (lines[0] !== '---') {
        return { frontmatter: {}, body: content };
    }
    let endIndex = -1;
    for (let i = 1; i < lines.length; i++) {
        if (lines[i] === '---') {
            endIndex = i;
            break;
        }
    }
    if (endIndex === -1) {
        throw new Error('Invalid frontmatter: closing --- not found');
    }
    const frontmatterStr = lines.slice(1, endIndex).join('\n');
    const frontmatter = yaml_1.default.parse(frontmatterStr) || {};
    const body = lines.slice(endIndex + 1).join('\n');
    return { frontmatter, body };
}
/**
 * Create YAML frontmatter string
 */
function createFrontmatter(data) {
    return `---\n${yaml_1.default.stringify(data)}---`;
}
/**
 * Write file with frontmatter + body
 */
async function writeMarkdownFile(filepath, frontmatter, body) {
    const content = `${createFrontmatter(frontmatter)}\n${body}`;
    await promises_1.default.writeFile(filepath, content, 'utf-8');
}
/**
 * Read markdown file with metadata
 */
async function readMarkdownFile(filepath) {
    const content = await promises_1.default.readFile(filepath, 'utf-8');
    const { frontmatter, body } = parseFrontmatter(content);
    const stat = await promises_1.default.stat(filepath);
    const filename = path_1.default.basename(filepath);
    const id = frontmatter.id || filename.replace('.md', '');
    return {
        id,
        title: frontmatter.title,
        filename,
        path: filepath,
        content,
        frontmatter,
        modified: stat.mtime.toISOString(),
    };
}
/**
 * List files in directory
 */
async function listMarkdownFiles(dirPath) {
    try {
        const files = await promises_1.default.readdir(dirPath, { recursive: true });
        const mdFiles = files
            .filter((f) => typeof f === 'string' && f.endsWith('.md'))
            .sort();
        const results = [];
        for (const file of mdFiles) {
            const fullPath = path_1.default.join(dirPath, file);
            try {
                const metadata = await readMarkdownFile(fullPath);
                results.push(metadata);
            }
            catch (err) {
                console.warn(`Skipping invalid file ${fullPath}:`, err);
            }
        }
        return results;
    }
    catch (err) {
        if (err.code === 'ENOENT') {
            return [];
        }
        throw err;
    }
}
/**
 * Ensure directory exists
 */
async function ensureDir(dirPath) {
    try {
        await promises_1.default.mkdir(dirPath, { recursive: true });
    }
    catch (err) {
        if (err.code !== 'EEXIST') {
            throw err;
        }
    }
}
/**
 * Delete a file
 */
async function deleteFile(filepath) {
    await promises_1.default.unlink(filepath);
}
/**
 * File managers for each type
 */
exports.specsManager = {
    async list() {
        return listMarkdownFiles(path_1.default.join(CONFIG_ROOT, 'spec'));
    },
    async get(id) {
        const files = await this.list();
        const file = files.find((f) => f.id === id);
        if (!file)
            throw new Error(`Spec ${id} not found`);
        return readMarkdownFile(file.path);
    },
    async create(data) {
        const id = `SPEC-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`;
        const filename = `${id}-${data.title.toLowerCase().replace(/\s+/g, '-')}.md`;
        const filepath = path_1.default.join(CONFIG_ROOT, 'spec', filename);
        const frontmatter = {
            id,
            title: data.title,
            phase: data.phase,
            status: 'draft',
            version: '1.0',
            created: new Date().toISOString().split('T')[0],
            owner: data.owner,
            priority: data.priority || 'medium',
        };
        const body = `# Spécification: ${data.title}\n\n## Description\n${data.description || ''}\n\n## Critères d'Acceptation\n${(data.criteria || []).map((c) => `- [ ] ${c}`).join('\n')}\n`;
        await ensureDir(path_1.default.dirname(filepath));
        await writeMarkdownFile(filepath, frontmatter, body);
        return readMarkdownFile(filepath);
    },
    async update(id, data) {
        const file = await this.get(id);
        const { frontmatter, body } = parseFrontmatter(file.content);
        Object.assign(frontmatter, data);
        frontmatter.modified = new Date().toISOString().split('T')[0];
        await writeMarkdownFile(file.path, frontmatter, body);
        return readMarkdownFile(file.path);
    },
    async delete(id) {
        const file = await this.get(id);
        await deleteFile(file.path);
    },
};
exports.promptsManager = {
    async list() {
        const phases = ['dev', 'test', 'deploy'];
        const allPrompts = [];
        for (const phase of phases) {
            const promptsDir = path_1.default.join(CONFIG_ROOT, phase, 'prompts');
            const prompts = await listMarkdownFiles(promptsDir);
            allPrompts.push(...prompts);
        }
        return allPrompts;
    },
    async listByPhase(phase) {
        const promptsDir = path_1.default.join(CONFIG_ROOT, phase, 'prompts');
        return listMarkdownFiles(promptsDir);
    },
    async listByRole(role) {
        const files = await this.list();
        return files.filter((f) => f.frontmatter.role?.toLowerCase() === role.toLowerCase());
    },
    async get(id) {
        const files = await this.list();
        const file = files.find((f) => f.id === id || f.filename.replace('.md', '') === id);
        if (!file)
            throw new Error(`Prompt ${id} not found`);
        return readMarkdownFile(file.path);
    },
    async create(data) {
        const filepath = path_1.default.join(CONFIG_ROOT, data.phase, 'prompts', `${data.filename}.md`);
        const frontmatter = {
            role: data.role,
            phase: data.phase,
            action: data.action,
            version: data.version || '1.0',
            triggers: data.triggers || [],
        };
        await ensureDir(path_1.default.dirname(filepath));
        await writeMarkdownFile(filepath, frontmatter, data.body);
        return readMarkdownFile(filepath);
    },
    async update(id, data) {
        const file = await this.get(id);
        const { frontmatter, body } = parseFrontmatter(file.content);
        Object.assign(frontmatter, data);
        await writeMarkdownFile(file.path, frontmatter, body);
        return readMarkdownFile(file.path);
    },
    async delete(id) {
        const file = await this.get(id);
        await deleteFile(file.path);
    },
};
exports.maestrosManager = {
    async list() {
        return listMarkdownFiles(MAESTROS_ROOT);
    },
    async listByPhase(phase) {
        const files = await this.list();
        return files.filter((f) => f.frontmatter.phase === phase);
    },
    async get(id) {
        const files = await this.list();
        const file = files.find((f) => f.id === id);
        if (!file)
            throw new Error(`Maestro ${id} not found`);
        return readMarkdownFile(file.path);
    },
    async create(data) {
        const id = data.name.toLowerCase().replace(/\s+/g, '-');
        const filepath = path_1.default.join(MAESTROS_ROOT, data.category, `${id}.md`);
        const frontmatter = {
            id,
            name: data.name,
            phase: data.phase,
            expertise: data.expertise,
            version: data.version || '1.0',
            created: new Date().toISOString().split('T')[0],
        };
        await ensureDir(path_1.default.dirname(filepath));
        await writeMarkdownFile(filepath, frontmatter, data.body);
        return readMarkdownFile(filepath);
    },
    async update(id, data) {
        const file = await this.get(id);
        const { frontmatter, body } = parseFrontmatter(file.content);
        Object.assign(frontmatter, data);
        await writeMarkdownFile(file.path, frontmatter, body);
        return readMarkdownFile(file.path);
    },
    async delete(id) {
        const file = await this.get(id);
        await deleteFile(file.path);
    },
};
exports.workflowsManager = {
    async list() {
        return listMarkdownFiles(WORKFLOWS_ROOT);
    },
    async get(id) {
        const files = await this.list();
        const file = files.find((f) => f.id === id);
        if (!file)
            throw new Error(`Workflow ${id} not found`);
        return readMarkdownFile(file.path);
    },
    async create(data) {
        const filepath = path_1.default.join(WORKFLOWS_ROOT, `${data.filename}.md`);
        const id = data.filename.toUpperCase().replace(/-/g, '_');
        const frontmatter = {
            id,
            title: data.title,
            version: data.version || '1.0',
            created: new Date().toISOString().split('T')[0],
        };
        await ensureDir(path_1.default.dirname(filepath));
        await writeMarkdownFile(filepath, frontmatter, data.body);
        return readMarkdownFile(filepath);
    },
    async update(id, data) {
        const file = await this.get(id);
        const { frontmatter, body } = parseFrontmatter(file.content);
        Object.assign(frontmatter, data);
        await writeMarkdownFile(file.path, frontmatter, body);
        return readMarkdownFile(file.path);
    },
    async delete(id) {
        const file = await this.get(id);
        await deleteFile(file.path);
    },
};
